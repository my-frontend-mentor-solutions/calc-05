    ...
    return (
        <>
            <div className='bg-screen-back-light dark:bg-screen-back-dark py-1 px-2 rounded-md text-right mb-3 h-[100px]'>
                ...
                <p className='font-bold text-[32px] overflow-hidden'>{result ? result : expression}</p>
            </div>
            <div className='bg-keypad-back-light dark:bg-keypad-back-dark rounded-md p-1'>
                <div className='grid grid-cols-4'>
                    ...
                    <ImpKeys value={'='} clickHandler={onResult} width />
                </div>
            </div>
        </>
    )
...
const ImpKeys = (props) => {
    ...
    const width = props.width ? 'col-span-2	' : '';

    return (
        <button className={`text-white bg-imp-back-light dark:bg-imp-back-dark shadow-blue-light dark:shadow-blue-dark hover:bg-imp-hover-light dark:hover:bg-imp-hover-dark h-[60px] ${width} flex items-center text-center justify-center rounded-md m-[6px] duration-200 hover:cursor-pointer `} value={props.value} onClick={(e) => props.clickHandler(e.target.value)}>
            {props.value}
        </button>
    )
}